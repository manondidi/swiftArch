...
class SPFeedToolbar: UIView {
    ...
    private lazy var repostButton: UIButton = {
        var repostButton = self.composeButton(icon:UIImage(named: "timeline_icon_retweet"))
        return repostButton
    }()
    
    private lazy var commentButton: UIButton = {
        var commentButton = self.composeButton(icon:UIImage(named: "timeline_icon_comment"))
        return commentButton
    }()
    
    private lazy var likeButton: UIButton = {
        var likeButton = self.composeButton(icon: UIImage(named: "timeline_icon_unlike"))
        return likeButton
    }()
    ...
    private func composeButton(icon: UIImage?) -> UIButton {
        ...
        button.setBackgroundImage(UIImage(named:"timeline_card_bottom_background_highlighted"), for: UIControl.State.highlighted)
    ...
    private func setCount(button: UIButton, count c: Int?) {
        if let count = c {
            if count > 0 {
                button.setTitle("\(count)", for: UIControl.State.normal)
            } else {
