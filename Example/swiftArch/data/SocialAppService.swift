...
class SocialAppService {
    ...
    let httpClient = HttpClient(baseUrl: DataManager.baseUrlSocail, headers: ["X-Requested-With": "XMLHttpRequest"])
    ...
    private func checkSuccess<T>(result: Result<T>)throws {
        if(!result.isSuccess) {
            throw ResultError(code: result.status ?? -1, msg: result.msg)
        }
    }
